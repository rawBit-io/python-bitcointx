...
- **Script vectors**: `test_script_vectors.py` checks `VerifyScript` against Bitcoin Core’s `script_tests.json` (v30; legacy, P2SH, SegWit v0). Taproot template success cases are `xfail` because placeholders like `#SCRIPT#`/`#CONTROLBLOCK#` are not expanded; expected-failure template cases remain active to catch wrong accepts. Data lives in `data/script_tests.json`. Run with `pytest bitcointx/tests_core/test_script_vectors.py -vv`. Update vectors from a Core checkout via `cp src/test/data/script_tests.json bitcointx/tests_core/data/`.
